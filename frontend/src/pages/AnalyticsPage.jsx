...
export default function AnalyticsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [classroomData, questionData, submissionsData] =
          await Promise.all([
  ...
  return (
    ...
    >
      ...
      <main style={{ flex: 1, overflowY: "auto", padding: "2rem 2rem 3rem" }}>
          ...
          >
            <div>
              ...
              >
                ...
                <span
                  className={`badge ${diffColors[question.difficulty] || "badge-medium"}`}
                >
                  {question.difficulty}
                </span>
                <span
            ...
            <button
              ...
              onClick={() =>
                navigate(
                  `/teacher/classroom/${classroomId}/question/${qid}/edit`,
                  { state: { question } },
                )
              }
        ...
        {/* ML Error Categories */}
        {submissions.filter(s => s.error_category).length > 0 && (
          (() => {
            const categoryCounts = {};
            submissions.forEach(s => {
              if (s.error_category) {
                categoryCounts[s.error_category] = (categoryCounts[s.error_category] || 0) + 1;
              }
            });
            ...
            const categoryColors = {
              ...
              'Runtime Error': 'var(--color-tertiary-container)',
              'Logical Error': 'var(--color-secondary-container)',
              'Inefficient Solution': 'var(--color-primary-container)',
              'Correct': 'var(--color-console-success)',
            };

            return (
              <div className="card" style={{ padding: '1.25rem', marginBottom: '1.5rem' }}>
                <div style={{ fontFamily: 'var(--font-grotesk)', fontSize: '0.6875rem', fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase', color: 'var(--color-on-surface-variant)', marginBottom: '1rem' }}>
                  ML Error Categories
                </div>
                {sortedCategories.map(([category, count]) => {
                  const pct = (count / maxCount) * 100;
                  const color = categoryColors[category] || 'var(--color-primary-container)';
                  return (
                    <div key={category} style={{ marginBottom: '0.75rem' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.25rem', fontSize: '0.8125rem' }}>
                        <span style={{ fontWeight: 500, color: 'var(--color-on-surface)' }}>{category}</span>
                        <span style={{ fontFamily: 'var(--font-grotesk)', fontWeight: 600, color: 'var(--color-on-surface-variant)' }}>{count}</span>
                      </div>
                      <div style={{ background: 'var(--color-surface-container)', borderRadius: 'var(--radius-full)', height: '8px', overflow: 'hidden' }}>
            ...
            return (
              <div
                ...
                style={{
                  padding: "1rem 0",
                  borderBottom:
                    idx !== group.submissions.length - 1
                      ? "1px solid var(--color-outline-variant)"
                      : "none",
                }}
              >
                ...
                >
                  Attempt #{group.submissions.length - idx} ·{" "}
                  {new Date(sub.created_at).toLocaleString()}
                  {feedback && (
                    <span
                      className={`badge ${feedback.status === "passed" ? "badge-active" : feedback.status === "error" ? "badge-archived" : "badge-medium"}`}
